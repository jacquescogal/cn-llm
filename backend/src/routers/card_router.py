...
async def get_cards_for_word(word_id: int) -> Optional[ReadCardDto]:
    return await card_controller.get_cards_for_word_singular(word_id)
...
@card_router.post("/card/word/{word_id}/type/{card_type}/review/{review_type}", tags=["card"])
async def create_card_of_word_card_type(word_id: int, card_type: int, review_type: int) -> Optional[ReadCardDto]:
    return await card_controller.create_card_of_word_card_type_review_type(word_id, CardType(card_type), ReviewType(review_type))

@card_router.delete("/card/word/{word_id}/type/{card_type}/review/{review_type}", tags=["card"])
async def delete_card_of_word_card_type_review_type(word_id: int, card_type: int, review_type: int) -> Optional[ReadCardDto]:
    return await card_controller.delete_card_of_word_card_type_review_type(word_id, CardType(card_type), ReviewType(review_type))
